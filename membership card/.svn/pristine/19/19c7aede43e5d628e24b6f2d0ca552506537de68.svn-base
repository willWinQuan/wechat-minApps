...
function commonRequest(data, url, callback) {
  wx.request({
    ...
    complete: function (msg) {
      console.log(msg)
      // console.log(msg.data.user_id_en)
      // var user_id_en = msg.data.user_id_en;      
      // complete  
    }
  })
}
...
function topColor(){
  wx.request({
    url: config.http_host + '/mini_program/wa_card/back/index.php/home/miniprogram/color_style',
    data: {
      customer_id_en: config.customer_id_en,
    },
    success: function (res) {
      ...
      if (res.data.data == ''){
        ...
        var color=res.data.data;
        console.log(color)
        if (res.data.data == '#FFFFFF') {
          wx.setNavigationBarColor({
            frontColor: '#000000',
            backgroundColor: res.data.data,
          })
        }else{
          wx.setNavigationBarColor({
            frontColor: '#ffffff',
...
function share(callback) {
  wx.request({
    url: config.http_host + '/mini_program/wa_card/back/index.php/home/miniprogram/share_img',
    data: {
      customer_id_en: config.customer_id_en,
    },
    success: function (res) {
