...
Page({
  ...
  onShow: function () {
    ...
    // 只支持1.6.0以上的版本
    if (util.version>=160){
      console.log(util.innerAudioContext)
  ...
  details:function(){
    ...
    // 请求首页详情
    wx.request({
      url: http_host + '/mini_program/applets/index.php/answer/Front_expert_order/order_list',
      data: {
      ...
      success: function (res) {
        ...
        if (list.length == 0){
          ...
          console.log(list[0].question_users.id)
          if (list.length<10){
            setTimeout(function () {
              that.setData({
                list: arr,
  ...
  details1: function () {
    ...
    // 请求详情
    wx.request({
      url: http_host + '/mini_program/applets/index.php/answer/Front_expert_order/get_order_count',
      data: {
        customer_id_en: customer_id_en,
        user_id_en: user_id_en,
      },
  ...
  recordBtn() {
    console.log("录音")
    if (this.data.recordShow==0){
      this.setData({
        recordShow:1
      })
      
    }
    if (this.data.recordShow == 1) {
      this.setData({
        recordShow: 2
      })
      
    }
    if (this.data.recordShow == 2) {
      this.setData({
        recordShow: 0
      })
    ...
    this.setData({
      recordContr: !this.data.recordContr
    })
  },
  // 选择分类
  bindTap(e) {
  ...
  voice: function (e) {
    ...
    util.innerAudioContext.onEnded((res) => {
      ...
      that.data.list[index].answer.seconds_time = 0;
  ...
  navigator:function(e){
    ...
    if (that.data.Jump == 1) {
      ...
      wx.navigateTo({
        url: '../questions_send_txt/questions_send_txt?question_id=' + question_id,
      })
    }
  },
  // 触碰到底部
